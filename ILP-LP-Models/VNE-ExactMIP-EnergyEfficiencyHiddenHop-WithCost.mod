      ...
      /* objective function */
      minimize min: w1*((sum{i in SNUnmappedNodes} alpha[i]) * 252 + (sum{(i,j) in SNUnmappedLinks} rho[i,j])*4) + w2*((sum{(i,j) in SNtotal}sum{(k,l) in SNlinks} (1/(LinkCapacity[k,l]+w3))*flow[i,j,k,l]) + (sum{i in SNnodes}((1/(SNodeCapacity[i]+w3))*(sum{k in VNnodes}x[k,i]*VNodeDemand[k]))));
