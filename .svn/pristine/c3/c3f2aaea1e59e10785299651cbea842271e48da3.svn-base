...
public class CoordinatedMappingRoundingPathStrippingWizard extends
	...
	public CoordinatedMappingRoundingPathStrippingWizard(Scenario scenario) {
	...
	protected JPanel createContent() {
		...
		snHorizontal.addGroup(snLayout.createParallelGroup()
				...
				.addComponent(bwWLabel).addComponent(kLabel)
				.addComponent(typeLabel).addComponent(nodeOverloadLabel));
	...
	protected void doAction() {
		...
		IAlgorithm coordinatedRoundStripping = null;
		...
		if (!deterministicButton.isSelected() && !randomizedButton.isSelected()) {
			...
			coordinatedRoundStripping = new CoordinatedMappingRoundingPathStripping(
					ns, distance, wCpu, wBw, 1, k, true);
		} else if (randomizedButton.isSelected()
				&& noNodeOverloadButton.isSelected()) {
			coordinatedRoundStripping = new CoordinatedMappingRoundingPathStripping(
					ns, distance, wCpu, wBw, 1, k, false);
		} else if (deterministicButton.isSelected()
				&& yesNodeOverloadButton.isSelected()) {
			coordinatedRoundStripping = new CoordinatedMappingRoundingPathStripping(
					ns, distance, wCpu, wBw, 0, k, true);
		} else if (deterministicButton.isSelected()
				&& noNodeOverloadButton.isSelected()) {
			coordinatedRoundStripping = new CoordinatedMappingRoundingPathStripping(
					ns, distance, wCpu, wBw, 0, k, false);
		}
		new MyProgressBarDialog(coordinatedRoundStripping, scenario);
