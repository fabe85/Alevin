...
public class BreadthFirstSearchTest {
	...
	public void bfsTest() {
		...
		int i = 1;
		for (SubstrateLink l : subsNetwork.getEdges()) {
			newSNet.put(l, (double) i++);

		}

		Map<SubstrateLink, Double> mtmp = new LinkedHashMap<SubstrateLink, Double>();
		Map<SubstrateLink, Double> smtmp = new LinkedHashMap<SubstrateLink, Double>();
		Map<List<SubstrateLink>, Double> candidatePaths = new LinkedHashMap<List<SubstrateLink>, Double>();
		Map<SubstrateLink, Double> updatedNewSNet = new LinkedHashMap<SubstrateLink, Double>();

		BreadthFirstSearch bfs = new BreadthFirstSearch(newSNet.keySet(), 1, 0,
				subsNetwork);
		assertNotNull(bfs);

		List<SubstrateLink> tSPath = bfs.search();
		// assertNotNull(bfs.search());
		System.out.println("FINAL:" + tSPath);
		...
		// Find the lowest weight in the path
		for (SubstrateLink l : tSPath) {
			for (Iterator<SubstrateLink> it = newSNet.keySet().iterator(); it
					.hasNext();) {
				SubstrateLink tsl = it.next();
				Double tv = newSNet.get(tsl);
				if (l == tsl) {
					mtmp.put(tsl, tv);
				}
			}
		}
		System.out.println("Temporal map:" + mtmp);
		smtmp = MiscelFunctions.sortByValue(mtmp);

		System.out.println("Temporal sorted map:" + smtmp);

		// Get the last element (lowest weight)
		Double pathW = (double) 0;
		int i1 = 0;
		for (Iterator<SubstrateLink> it = smtmp.keySet().iterator(); it
				.hasNext();) {
			SubstrateLink tsl = it.next();
			if (i1 == (smtmp.keySet().size() - 1)) {
				pathW = smtmp.get(tsl);
			}
			i1++;
		}
		...
		System.out.println("New candidate path:" + candidatePaths);

		System.out.println("Old newSNet:" + newSNet);
		// Update weights of newSNet
		for (Iterator<SubstrateLink> itt = newSNet.keySet().iterator(); itt
				.hasNext();) {
			SubstrateLink tsll = itt.next();
			Double tv = newSNet.get(tsll);
			updatedNewSNet.put(tsll, tv);
		}

		for (Iterator<SubstrateLink> it = mtmp.keySet().iterator(); it
				.hasNext();) {
			SubstrateLink tsl = it.next();
			...
			for (Iterator<SubstrateLink> itt = newSNet.keySet().iterator(); itt
				...
				Double tv = newSNet.get(tsll);
				if (tsl == tsll) {
					updatedNewSNet.remove(tsll);
					updatedNewSNet.put(tsll, tv - pathW);
				}
			}
		}
		System.out.println("New newSNet:" + updatedNewSNet);

		for (Iterator<SubstrateLink> itl = newSNet.keySet().iterator(); itl
				.hasNext();) {
			SubstrateLink tsll = itl.next();
			Double tv = updatedNewSNet.get(tsll);
			if (tv == 0) {
				updatedNewSNet.remove(tsll);
			}
		}

		// newSNet = updatedNewSNet;
		System.out.println("New newSNet without 0 values:" + newSNet);

		System.out.println("CANDITATE PATHS:" + candidatePaths);
	}
	...
	//
	// SubstrateNode subsNode2 = new SubstrateNode();
	// idRes = new IdResource(subsNode2);
	// idRes.setId(Integer.toString(2));
	// assertTrue(subsNode2.add(idRes));
	// cpuRes = new CpuResource(subsNode2);
	// cpuRes.setCycles(100.0);
	...
	//
	// SubstrateNode subsNode3 = new SubstrateNode();
	// idRes = new IdResource(subsNode3);
	// idRes.setId(Integer.toString(3));
	// assertTrue(subsNode3.add(idRes));
	// cpuRes = new CpuResource(subsNode3);
	// cpuRes.setCycles(100.0);
	...
	//
	// SubstrateNode subsNode4 = new SubstrateNode();
	// idRes = new IdResource(subsNode4);
	// idRes.setId(Integer.toString(4));
	// assertTrue(subsNode4.add(idRes));
	// cpuRes = new CpuResource(subsNode4);
	// cpuRes.setCycles(100.0);
	...
	//
	// SubstrateNode subsNode5 = new SubstrateNode();
	// idRes = new IdResource(subsNode5);
	// idRes.setId(Integer.toString(5));
	// assertTrue(subsNode5.add(idRes));
	// cpuRes = new CpuResource(subsNode5);
	// cpuRes.setCycles(100.0);
	...
	// idRes = new IdResource(subsNode6);
	...
	// idRes = new IdResource(subsNode7);
	...
	// idRes = new IdResource(subsNode8);
